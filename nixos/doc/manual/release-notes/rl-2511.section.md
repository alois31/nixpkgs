...
- Libvirt now uses the OVMF binaries shipped by QEMU for UEFI virtual machines. The options under `virtualisation.libvirtd.qemu.ovmf` have been removed. In addition, the firmware file paths have changed, necessitating manual changes in the XML files of existing virtual machines using UEFI boot:
  ...
  Due to a bug that has been fixed, some recently created UEFI virtual machines may also refer to firmware paths under `/nix/store/…-qemu-…/share/qemu`, which will break at latest when these paths are garbage collected. This directory should be modified to `/run/libvirt/nix-ovmf` as well while keeping the filename.
